...
def predict_employee_attrition(token: str, employee_data: dict) -> dict:
    ...
    try:
        response = requests.post(f"{API_URL}/predict", headers=headers, json=employee_data, timeout=10)
        response.raise_for_status()
...
def get_employee_data():
    ...
    return {
        ...
        "NumCompaniesWorked": st.number_input("Number of Companies Worked", min_value=1),
        "WorkExperience": st.number_input("Total Years of Work Experience", min_value=0.0),
        "Department": st.selectbox("Department", ["Human Resources", "Research & Development", "Sales"]),
        "JobRole": st.selectbox("Job Role", ["Manager", "Sales Executive", "Healthcare Representative"]),
        "JobLevel": st.number_input("Job Level (1-5)", min_value=1, max_value=5),
        "DistanceFromHome": st.number_input("Distance from Home (miles)", min_value=0.0),
        "BusinessTravel": st.selectbox(
            "Business Travel Frequency",
            ["Non-Travel", "Travel_Rarely", "Travel_Frequently"],
        ),
        "OverTime": st.selectbox("Works Overtime?", ["Yes", "No"]),
        "JobInvolvement": st.number_input("Job Involvement Level (1-4)", min_value=1, max_value=4),
        "PercentSalaryHike": st.number_input("Percentage of Salary Hike (0-100)", min_value=0.0, max_value=100.0),
        ...
        "StockOptionLevel": st.number_input("Stock Option Level (0-3)", min_value=0, max_value=3),
        "PerformanceRating": st.number_input("Performance Rating (1-4)", min_value=1, max_value=4),
        "TrainingTimesLastYear": st.number_input("Training Times Last Year", min_value=0),
        "YearsWithCurrManager": st.number_input("Years with Current Manager", min_value=0.0),
        "OverallSatisfaction": st.number_input("Overall Job Satisfaction (1-5)", min_value=1.0, max_value=5.0),
    }
